...
describe('SpecialMatchables.AreaClear5x5WhenMatched', () => {
	it('is formed when a T match is performed, and clears everything next to it when matched', () => {
		...
		TestUtil.expectGridSize(simulation.grid, [2, 2, 1, 3, 3, 3, 3]);

		//Check the matchable looks right
		let m = simulation.grid.cells[2][0];
		expect(m.isDisappearing).toBe(false);
		expect(m.type).toBe(Type.AreaClear5x5WhenMatched);
		...
		//Check the grid, only 8,9s should remain
		TestUtil.expectGridSize(simulation.grid, [2, 0, 0, 0, 0, 0, 3]);

		TestUtil.expectGridQuiet(simulation);
		scoreEarnedChecker.expectScore(120);
